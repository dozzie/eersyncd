...
init([Addr, Port] = _Args) ->
  eersyncd_log:set_context(connection, [
  ...
  case resolve(Addr) of
    {ok, BindAddr} ->
      case listen(BindAddr, Port) of
        {ok, Socket} ->
          eersyncd_log:info("listening on socket",
          ...
          {ok, State, 0};
        {error, Reason} ->
          eersyncd_log:err("can't setup listening socket", [
            {action, setup},
            {error, {term, Reason}},
            {bind_address, format_address(BindAddr)}
          ]),
    ...
    {error, Reason} ->
      eersyncd_log:err("can't resolve bind address",
...
handle_call(rebind = _Request, _From, State) ->
  case rebind_state(State) of
    ...
    {ok, NewState = #state{bind_address = NewBindAddr}} ->
      eersyncd_log:info("rebound to a new address",
...
handle_call(shutdown = _Request, _From, State) ->
  eersyncd_log:info("shutting down listening socket"),
...
handle_info(timeout = _Message, State = #state{socket = Socket}) ->
  case gen_tcp:accept(Socket, ?ACCEPT_LOOP_INTERVAL) of
    {ok, Client} ->
      eersyncd_tcp_conn:take_over(Client),
      {noreply, State, 0};
    {error, timeout} ->
      % OK, no incoming connection
      {noreply, State, 0};
    {error, Reason} ->
      eersyncd_log:err("accept error", [{error, {term, Reason}}]),
...
-spec format_address(eersyncd:address(), inet:port_number()) ->
  binary().
...
-spec format_address(eersyncd:address()) ->
  binary().
