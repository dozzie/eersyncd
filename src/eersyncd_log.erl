...
set_context(EventType, EventInfo) ->
  put('$eersyncd_log', {EventType, EventInfo}),
  ok.
...
append_context(EventInfo) ->
  {EventType, EventContext} = get('$eersyncd_log'),
  put('$eersyncd_log', {EventType, EventContext ++ EventInfo}),
  ok.
...
%%   error), but it's harmless for the operation of eersyncd application.
%%
%%   Function expects {@link set_context/2} to be called first.
