...
take_over(Socket) ->
  case eersyncd_tcp_conn_sup:spawn_worker(Socket) of
    {ok, Pid} ->
...
init([Socket] = _Args) ->
  ...
  case rsync_exec(RsyncPath, ConfigPath, LogPath, Cwd) of
    ...
    {error, Reason} ->
      ...
      eersyncd_log:warn("rsync spawn error", [
        {error, {term, Reason}},
        {rsync, {str, RsyncPath}},
...
handle_info({subproc_error, Subproc, Reason} = _Message,
            State = #state{rsyncd = Subproc}) ->
  eersyncd_log:warn("rsync port terminated abnormally",
                    [{error, {term, Reason}}]),
...
handle_info({subproc_terminated, Subproc, exit, ExitCode} = _Message,
            State = #state{rsyncd = Subproc}) ->
  eersyncd_log:warn("rsync process exited with an error",
                    [{exit_code, ExitCode}]),
  {stop, normal, State};
handle_info({subproc_terminated, Subproc, signal, {_, SigName}} = _Message,
            State = #state{rsyncd = Subproc}) ->
  eersyncd_log:warn("rsync process killed", [{signal, {term, SigName}}]),
  {stop, normal, State};
...
handle_info({tcp_error, Socket, Reason} = _Message,
            State = #state{socket = Socket, rsyncd = Subproc}) ->
  eersyncd_log:warn("TCP socket terminated abnormally",
                    [{error, {term, Reason}}]),
  subproc:close(Subproc, read_write),
  {noreply, State};
