...
reload() ->
  {ok, NewAddrs} = application:get_env(eersyncd, listen),
...
stop_child(Name, Pid) ->
  eersyncd_tcp_listen:shutdown(Pid),
...
reload_child(Name, Pid) ->
  case eersyncd_tcp_listen:rebind(Pid) of
    ok -> ok;
...
init([] = _Args) ->
  ...
  Children = [
    {eersyncd_tcp_conn_sup, {eersyncd_tcp_conn_sup, start_link, []},
      permanent, 1000, supervisor, [eersyncd_tcp_conn_sup]} |
...
listen_child(Address, Port) ->
  {child_name(Address, Port),
    {eersyncd_tcp_listen, start_link, [Address, Port]},
    transient, 1000, worker, [eersyncd_tcp_listen]}.

child_name(Address, Port) ->
  {eersyncd_tcp_listen, Address, Port}.
